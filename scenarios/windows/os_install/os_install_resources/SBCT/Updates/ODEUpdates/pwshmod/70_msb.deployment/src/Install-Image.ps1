...
function Install-Image {
	...
	$WiFiInstall = $true
        ...
        Write-Host "------------------------------"
        $BootMeDriveLetter = (get-volume -FriendlyName 'BOOTME' -erroraction ignore).DriveLetter
        $LocalInstallDrive = Get-CimInstance -ClassName Win32_Volume -Filter "DriveLetter = '$BootMeDriveLetter'"
