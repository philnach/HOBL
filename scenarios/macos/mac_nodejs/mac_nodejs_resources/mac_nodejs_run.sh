...
METRICS_FILE="$LOG_DIR/mac_nodejs_results.csv"
LOG_FILE="$LOG_DIR/mac_nodejs_run.log"
...
echo "-- mac_nodejs_run.sh started $(date)" > "$LOG_FILE"
log "-- nodejs run started"
...
# Set Python version for Node.js build
...
log "-- Changing directory to: $BIN_DIR/node-25.0.0"

# Navigate to nodejs source directory
if [ ! -d "$BIN_DIR/node-25.0.0" ]; then
    log " ERROR - Node.js source directory not found: $BIN_DIR/node-25.0.0"
    log "Please run mac_nodejs_prep.sh first"
    exit 1
fi

cd $BIN_DIR/node-25.0.0 || {
    log " ERROR - Failed to change to $BIN_DIR/node-25.0.0"
    exit 1
}
log "✓ Current directory: $(pwd)"

log "-- Cleaning previous build"
make clean || log "No previous build to clean"

# Remove build directories to ensure clean state
log "-- Removing build directories"
rm -rf out Release Debug
log "✓ Removed build directories"

log "-- Configuring Node.js build"
# Configure Node.js build (this generates the Makefile)
./configure --enable-static
check_status "Node.js configure"

log "-- Starting Node.js compilation"
# Build Node.js - use multiple cores for faster compilation
...
parse_time_output "$LOG_DIR/mac_nodejs_build_time.log" "build"

log "-- nodejs build ended"
